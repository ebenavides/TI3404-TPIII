...
fun validatupla (hd list2tuple: int*int*int)=
    if (#2 (hd list2tuple)) = 02
    then esbisiesto(#3 (hd list2tuple)) = true
	if (#1(hd list2tuple)) < 30 
	then true 
	else false
    else 
	if (#2 (hd list2tuple)) = 04 orelse (#2 (hd list2tuple)) = 06 orelse (#2 (hd list2tuple)) = 09 orelse (#2 (hd list2tuple))= 11
        then
            if (#1 (hd list2tuple)) < 31
	    then true
	    else false
	else 
	    if (#2 (hd list2tuple)) = 01 orelse (#2 (hd list2tuple)) = 03 orelse (#2 (hd list2tuple)) = 05 orelse (#2 (hd list2tuple)) = 07 orelse (#2 (hd list2tuple))= 08 orelse (#2 (hd list2tuple)) = 10 orelse (#2 (hd list2tuple))= 12
	    then
		if (#1 (hd list2tuple)) < 32
		then true
		else false
  else false


let fun aux (hd list2tuple: int*int*int, tl list2tuple:(int*int*int)list, lisresul:(int*int*int)list)=
...
fun aux (tl lisresul:(int*int*int)list, hd lisresul: int*int*int)=
    if null (tl liresul)
    then (hd lisresul)
    else if comparatupla((hd lisresul), hd (tl lisresul))
    then aux(tl lisresul, hd lisresul)
    else
	aux (tl lisresul, hd lisresul)

fun buscaviejo (lisresul)=
    if null lisresul
    then lisresul
    else
	aux (tl lisresul, hd lisresul)
